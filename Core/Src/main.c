      ...
      if(rxLen > 0)
      {
          ...
          if(newlinePos)
          {
              ...
              if (strncmp((char*)rxBuffer, "PWD:", 4) == 0)
              {
                  ...
                  snprintf((char*)txBuffer, TX_BUFFER_SIZE, "OK:%.100s\n", rxBuffer+4);
              ...
              {
                  snprintf((char*)txBuffer, TX_BUFFER_SIZE, "READ:%.100s\n", storedPassword);
              }
              else if (strncmp((char*)rxBuffer, "SEND", 4) == 0)
              {
            	  keyboardFlag = true;
                  snprintf((char*)txBuffer, TX_BUFFER_SIZE, "SEND:%.100s\n", storedPassword);
              }
              else
              {
                  snprintf((char*)txBuffer, TX_BUFFER_SIZE, "ERR:%.100s\n", rxBuffer);
              }
